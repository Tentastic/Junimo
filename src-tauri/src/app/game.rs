...
pub fn start_game(app_handle: AppHandle, app_state: State<'_, AppState>) {
    ...
    match init_game(app_handle.clone(), app_state.stop_game.clone()) {
        Ok(_) => {
        },
        Err(e) => {
            console::add_line(&app_handle, format!("<span style=\"color: #c22f2f\">[Junimo] Failed to start game: {}</span>", e));
        }

    }
}

fn init_game(original_app_handle: AppHandle, original_stop_game: Arc<Mutex<bool>>) -> Result<(), String> {
    ...
    thread::spawn(move || {
        ...
        let mod_names: HashMap<String, String> = mods.iter()
            .filter_map(|mod_info| {
                match get_dir_name(mod_info) {
        ...
        let dirs_to_remove = directories.iter().filter(|dir| !mod_names.contains_key(*dir)).collect::<Vec<_>>();
        let mods_to_add = mod_names.iter()
            .filter_map(|(dir, name)| {
        ...
        install_missing_mods(&app_handle, &config, mods_to_add, &stop_game).map_err(|_| {
            console::add_line(&app_handle, "<span style=\"color: #c22f2f\">[Junimo] Failed to install mods</span>".to_string());
        }).unwrap_or_default();
...
fn get_dir_name(mod_info: &ModInfo) -> Result<String, String> {
    ...
    match zip_archive {
        ...
        Err(_) => {
            Err("Failed to open zip archive".to_string())
        }
    }
}
...
fn uninstall_mods(app_handle: &AppHandle, mod_path: &String, dir_to_remove: Vec<&String>) {
    for dir in dir_to_remove {
        ...
        match fs::remove_dir_all(dir_path) {
            Ok(_) => {
                console::add_line(&app_handle, format!("<span style=\"color: #c22f2f\">[Junimo] Uninstalled {}</span>", &dir));
            },
            Err(_) => {
                console::add_line(&app_handle, format!("<span style=\"color: #c22f2f\">[Junimo] Failed to uninstall {}</span>", &dir));
            }
        };
    }
}

fn install_missing_mods(app_handle: &AppHandle, config: &Config, mods_to_add: Vec<&ModInfo>, stop_game: &Arc<Mutex<bool>>) -> Result<(), String> {
    ...
    for mods in mods_to_add {
        ...
        let zip_file = File::open(&zip_file_path).map_err(|_| {
            console::add_line(&app_handle, format!("<span style=\"color: #c22f2f\">[Junimo] Failed to open {}</span>", mods.name));
        }).ok();
        if zip_file.is_none() {
            continue;
        }

        let zip_archive = ZipArchive::new(zip_file.unwrap()).map_err(|_| {
            console::add_line(&app_handle, format!("<span style=\"color: #c22f2f\">[Junimo] Failed to open {}</span>", mods.name));
        }).ok();
        ...
        match zips::extract_zip(zip_archive.unwrap(), &PathBuf::from(&game_mod_path)) {
            Ok(_) => {
                console::add_line(&app_handle, format!("<span style=\"color: #2fb565\">[Junimo] Installed {}</span>", mods.name));
            },
            Err(e) => {
                console::add_line(&app_handle, format!("<span style=\"color: #c22f2f\">[Junimo] Failed to install {}</span>", mods.name));
            }
        }
    }
